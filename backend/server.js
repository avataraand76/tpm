...
// GET /api/maintenance-schedule-detail/by-serial/:serial — Tra cứu máy theo Serial Number
app.get(
  "/api/maintenance-schedule-detail/by-serial/:serial",
  authenticateToken,
  async (req, res) => {
    try {
      const { serial } = req.params;
      ...
      if (!code) {
        return res
          .status(400)
          .json({ success: false, message: "Số serial không hợp lệ" });
      }
      ...
      if (!mrows.length) {
        return res.status(404).json({
          success: false,
          message: `Không tìm thấy máy với serial "${code}"`,
        });
      }
      ...
      console.error("Error looking up machine by serial:", error);
      res.status(500).json({ success: false, message: error.message });
    }
  }
);
